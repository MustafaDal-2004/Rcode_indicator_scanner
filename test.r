...
#now we have a list now we can check the macd on them

results2 <- data.frame(Symbol = character(),
                      RSI = numeric(),
                      Bollinger = numeric(),
                      i = numeric(),
                      stringsAsFactors = FALSE)
...
i=1
while (i != length(results$Symbol)) {
  data <- data_collecter(results$i[i])
  macd_vaules <- macd(data)
  if((macd_vaules$MACD_Signal == "Buy") & (macd_vaules$Base =="Below") & (macd_vaules$MACD >= macd_vaules$Signal)){
    results2 <- rbind(results2, data.frame(Symbol = sp500_stocks[i],
                                         RSI = rsi(data),
                                         Bollinger = bollinger_bands(data),
                                         i = i))
  } 
