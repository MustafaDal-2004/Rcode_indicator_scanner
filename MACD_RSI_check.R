...
data_collecter <- function(i) {
  ...
  # Fetch stock data
  stock_data <- getSymbols(symbol, from = date, to = date3, auto.assign = FALSE)
...
#data getter and checker
data_collecter_before <- function(i) {
  # Get the stock symbol dynamically
  symbol <- sp500_stocks[i]
...
while (i != 503) {
  ...
  data2 <- data_collecter_before(i)
  macd_vaules <- macd(data)
  macd_vaules_before <- macd(data2)
  rsi_vaule <- rsi(data)
  if((macd_vaules$MACD_Signal == "Buy") & (macd_vaules$Base =="Below") & (macd_vaules$MACD >= macd_vaules$Signal) & (macd_vaules_before$MACD <= macd_vaules_before$Signal) & (rsi_vaule >= 45)){
    results <- rbind(results, data.frame(Symbol = sp500_stocks[i],
                                         RSI = rsi(data),
                                         Bollinger = bollinger_bands(data)))
    url <- paste0("https://uk.finance.yahoo.com/chart/", sp500_stocks[i])
    browseURL(url)
